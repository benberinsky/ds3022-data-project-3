...
for symbol in symbols:
    for window in [10, 30, 60]:
        stock_roll = returns[symbol].rolling(window)
        market_roll = returns["market"].rolling(window)

        # Compute rolling beta for last window
        last_beta = compute_beta(stock_roll.mean(), market_roll.mean())
        betas[window][symbol] = last_beta
